...
async function resolveTenantId(
  ...
  // 4. From leader
  if (leaderId) {
    const { data: leader } = await supabase.from("lideres").select("tenant_id").eq("id", leaderId).maybeSingle();
...
serve(async (req) => {
  ...
  try {
    ...
    // ============ RESOLVE TENANT ID ============
    const resolvedTenantId = await resolveTenantId(supabase, requestTenantId, authenticatedUserId, contactId, leaderId);
    ...
    if (!settings || !settings.resend_api_key) {
      ...
      if (globalSettingsError || !globalSettings) {
        ...
        return new Response(JSON.stringify({ success: false, error: "Configurações de email não encontradas" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (!settings.resend_enabled) {
      return new Response(JSON.stringify({ success: false, error: "Integração de email está desabilitada" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!settings.resend_api_key) {
      return new Response(JSON.stringify({ success: false, error: "API Key do Resend não configurada" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!to || !emailRegex.test(to)) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: `Email inválido: "${to}". O email deve seguir o formato email@exemplo.com`,
        }),
    ...
    if (templateSlug || templateId) {
      ...
      if (resolvedTenantId && templateSlug) {
        ...
        if (tenantTemplate) {
          ...
          console.log(
            `[send-email] Using tenant template override for slug: ${templateSlug}, tenant: ${resolvedTenantId}`,
          );
        }
      }
      ...
      if (!template) {
        const query = supabase.from("email_templates").select("*").eq("is_active", true);
        ...
        if (templateError || !globalTemplate) {
          ...
          return new Response(JSON.stringify({ success: false, error: "Template de email não encontrado" }), {
            status: 404,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      if (!variables.politico || !variables.cargo) {
        let orgQuery = supabase.from("organization").select("nome, cargo");
    ...
    if (!finalHtml || !finalSubject) {
      return new Response(JSON.stringify({ success: false, error: "Conteúdo do email ou assunto não fornecido" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
